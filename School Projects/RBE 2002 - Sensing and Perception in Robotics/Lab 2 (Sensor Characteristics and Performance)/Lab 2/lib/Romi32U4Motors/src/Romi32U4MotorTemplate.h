    ...
    */
    void ControlMotorSpeed(void)
    {
        if(ctrlMode == CTRL_SPEED)
        {
            ...
            sumError += error;
            float iTerm = sumError * Ki;

            // Limit the max/min integral effort to 125% of the max/min possible motor effort
            if (iTerm > maxEffort) {
                iTerm = maxEffort * 1.25;
            } else if (iTerm < -maxEffort) {
                iTerm = -maxEffort * 1.25;
            }

            int16_t effort = Kp * error + iTerm;
