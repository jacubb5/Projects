...
/**
 * Sets the wheel speeds based on the current u and omega values in twist
 */
void Chassis::SetTwist(const Twist& twist)
{
    SetWheelSpeeds(twist.u - twist.omega, twist.u + twist.omega);
}
...
Twist Chassis::CalcOdomFromWheelMotion(void)
{
    ...
    float leftEncoder = leftMotor.speed / LEFT_TICKS_PER_CM / (CONTROL_LOOP_PERIOD_MS/1000.0);
    float rightEncoder = rightMotor.speed / RIGHT_TICKS_PER_CM / (CONTROL_LOOP_PERIOD_MS/1000.0);

    velocity.u = (leftEncoder + rightEncoder) / 2;
    velocity.omega = (rightEncoder - leftEncoder) / (ROBOT_RADIUS * 2);

#ifdef __NAV_DEBUG__
    TeleplotPrint("u", velocity.u);
    TeleplotPrint("omega", velocity.omega * 180/M_PI);
#endif
