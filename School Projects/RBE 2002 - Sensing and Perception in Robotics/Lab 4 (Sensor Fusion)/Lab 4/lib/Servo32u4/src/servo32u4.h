...
protected:
    ...
    /**
     * Checks if the servo is currently at its target position.
     */
    bool checkAtPosition(void)
    {
        bool retVal = false;
    ...
    void update(void) 
    {
        ...
        else if(abs(targetPos - currentPos) <= 100) currentPos = targetPos;
        else if(targetPos > currentPos) currentPos += 100;
        else if(targetPos < currentPos) currentPos -= 100;
