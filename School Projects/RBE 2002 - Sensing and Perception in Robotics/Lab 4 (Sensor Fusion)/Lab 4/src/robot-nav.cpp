...
{
    if (robotState == ROBOT_IDLE || trashToDestination)
    {
        iTarget = xCoord;
        jTarget = yCoord;
...
void Robot::BeginRamp(int speed){
    robotState = ROBOT_RAMP;
    chassis.BeginHeadingDrive(speed);
}
...
void Robot::HandleDistanceReading(void)
{
    ...
    if (robotState == ROBOT_SEARCHING)
    {
        if (output < 30)
        {
            garbageDistance = output;
            Serial.print("+++ TARGET (");
            Serial.print(garbageDistance);
            Serial.println("CM) AWAY");
            Serial.println("--> BANGIN' A U'IE");
            trashAlignment = true;
            foundTrash = 1;
...
void Robot::HandleIntersection(void)
{
    if (hasTrash)
    {
        chassis.BeginDeadReckoning(8);
        robotState = ROBOT_DEAD_RECKONING;
    }
    if (robotState != ROBOT_IDLE && robotState != ROBOT_DEAD_RECKONING && robotState != ROBOT_TURNING)
    {
...
void Robot::HandleTurnComplete(void)
{
    ...
    if (iGrid == iTarget && jGrid == jTarget && foundTrash == 0) {
        ...
        chassis.EnterLineFollowing(LINE_SPEED);
    } else if (trashAlignment) {
        chassis.BeginDeadReckoning(6 - garbageDistance); // 6cm is the distance between the Romi axle and ultrasonic sensor
        robotState = ROBOT_DEAD_RECKONING;
    } else {
        chassis.EnterLineFollowing(LINE_SPEED);
        robotState = ROBOT_DRIVING;
...
void Robot::HandleDeadReckoningComplete(void) {
    chassis.Stop();
    if (trashToDestination && (iGrid == iTarget && jGrid == jTarget))
    {
        setServoPosition(SERVO_LOW);
        EnterIdleState();
        trashToDestination = false;
    }
    else if (hasTrash)
    {
        hasTrash = 0;
        trashToDestination = true;
        BeginToCoordinates(0, 0);
    }
    else if (trashAlignment)
    {   
        setServoPosition(SERVO_HIGH);
        robotState = ROBOT_LIFTING;
        hasTrash = 1;
        trashAlignment = false;
    }
    else if (iGrid == iTarget && jGrid == jTarget)
    { // Enters idle if at the target coordinates
        // EnterIdleState();
        robotState = ROBOT_SEARCHING;
    }
    else
    { // Turns towards target coordinate (in i or j direction) if not at target
