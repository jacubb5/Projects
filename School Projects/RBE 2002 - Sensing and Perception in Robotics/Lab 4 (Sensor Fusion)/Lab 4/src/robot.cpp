...
bool onFloor = true;
/**
 * Handles the reading of the pitch angle of the Romi
 */
void Robot::HandlePitchAngle(float& pitchAngle, float& headingAngle){
    if (robotState == ROBOT_RAMP)
    {
        // Hysteresis for detecting angle
        if (abs(pitchAngle) > 10 && !onRamp)
        {
            onRamp = true;
            Serial.println("--> ON RAMP");
            chassis.BeginHeadingDrive(10);
            digitalWrite(13, 1);
        }
        else if (abs(pitchAngle) < 1 && onRamp)
        {
            onRamp = false;
            onFloor = !onFloor;
            if (onFloor) { // Checks if the Romi is on the floor or at the top of the ramp
                Serial.println("vvv AT BOTTOM");
                EnterIdleState();
            } else {
...
void Robot::RobotLoop(void) 
{
    ...
    if(chassis.CheckIMU(pitchAngle, headingAngle)) HandlePitchAngle(pitchAngle, headingAngle);
